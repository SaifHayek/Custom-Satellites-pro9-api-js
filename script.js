...
searchInput.setAttribute('onkeyup','search(this.value)')
...
function search(val){
    ...
    .then(data =>{
        ...
        let filteredData = data.customer_satellites.filter(ele => {
            return ele.id.toUpperCase().includes(val.toUpperCase())
        })
        let objFilter = {customer_satellites:filteredData};
        container.innerHTML = ''
        showData(objFilter)
    })
    .catch(error =>console.log(error))
